...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for (float i = 0.0; i < 5.0; i++) {
        ...
        vec3 col1 = 1.5*texture(iChannel0, uv1).rgb * palette(length(uv0)+iTime*0.4+i*0.3);
        vec3 col2 = 1.5*texture(iChannel0, uv2).rgb * palette(length(uv0)+iTime*0.4+i*0.3);
        vec3 col3 = 1.5*texture(iChannel0, uv3).rgb * palette(length(uv0)+iTime*0.4+i*0.3);
